...
class PathfindingGUI:
    ...
    def setup_ui(self):
        ...
        tk.Label(control_frame, text="SETTINGS", fg=START_COLOR, bg="#2C2C2C",
                 font=("Courier", 16, "bold")).pack(pady=10)

        self.algo_var = tk.StringVar(value="A*")
        tk.Radiobutton(control_frame, text="A* Search", variable=self.algo_var,
                      value="A*", bg="#2C2C2C", fg="white", selectcolor=BG_COLOR).pack(anchor="w")
        tk.Radiobutton(control_frame, text="Greedy BFS", variable=self.algo_var,
                      value="GBFS", bg="#2C2C2C", fg="white", selectcolor=BG_COLOR).pack(anchor="w")

        self.heur_var = tk.StringVar(value="Manhattan")
        tk.Radiobutton(control_frame, text="Manhattan", variable=self.heur_var,
                      value="Manhattan", bg="#2C2C2C", fg="white", selectcolor=BG_COLOR)\
            .pack(anchor="w", pady=(10, 0))
        tk.Radiobutton(control_frame, text="Euclidean", variable=self.heur_var,
                      value="Euclidean", bg="#2C2C2C", fg="white", selectcolor=BG_COLOR)\
            .pack(anchor="w")

        self.dynamic_var = tk.BooleanVar(value=False)
        tk.Checkbutton(control_frame, text="Dynamic Obstacles", variable=self.dynamic_var,
                      bg="#2C2C2C", fg=PATH_COLOR, selectcolor=BG_COLOR).pack(pady=10)

        tk.Button(control_frame, text="Generate Random Maze",
                  command=self.random_maze, bg=WALL_COLOR, fg="white").pack(fill=tk.X, pady=5)
        tk.Button(control_frame, text="Clear Grid",
                  command=self.init_grid, bg=WALL_COLOR, fg="white").pack(fill=tk.X, pady=5)
        tk.Button(control_frame, text="START SEARCH",
                  command=self.start_search, bg=PATH_COLOR, fg="black",
                  font=("Arial", 12, "bold")).pack(fill=tk.X, pady=20)

        tk.Label(control_frame, text="METRICS", fg=GOAL_COLOR, bg="#2C2C2C",
        ...
        self.lbl_time = tk.Label(control_frame, text="Time: 0 ms",
                                 bg="#2C2C2C", fg="white")
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                if self.grid[r][c].is_wall:
                    color = WALL_COLOR
                elif self.grid[r][c] == self.start_node:
                    color = START_COLOR
                elif self.grid[r][c] == self.goal_node:
                    color = GOAL_COLOR
    ...
    def random_maze(self):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                if random.random() < 0.3 and self.grid[r][c] not in (
    ...
    def start_search(self):
        ...
        heapq.heappush(self.open_set,
                      (self.start_node.f(self.algo_var.get() == "A*"),
                       self.start_node))
        self.closed_set = set()
